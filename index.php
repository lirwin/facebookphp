...
<?php
	...
	if ($user): //check for existing user id
		...
		foreach ($moviefriends_graph as $key => $value) {
			...
			$moviespath = '/'.$value['uid'].'/movies?fields=id,name,description,link,picture.type(square).height(100).width(100)';
			$movies_graph = $facebook->api($moviespath);
			
			foreach ($movies_graph['data'] as $moviekey => $movievalue) {
				echo '<li>';
				echo '<a href="'. $movievalue['link']. '" target="_blank">';
		...
		$loginUrl = $facebook->getLoginUrl(array(
			...
			'redirect_uri' => 'http://apps.facebook.com/li-viewsourcephp'
		));
